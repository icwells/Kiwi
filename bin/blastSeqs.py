...
-threads {} -blast6out {}ublastX.outfmt6").format(infile, dbdir, threads, outdir)
	try:
		us = Popen(split(cmd))
		us.communicate()
		if us.returncode == 0:
	...
	if cont == True:
		...
		cmd = ("./usearch -ublast {} -db {}viralRefSeq.fna -evalue 1e-5 \
-threads {} -strand both -blast6out {}ublastN.outfmt6").format(query, dbdir, threads, outdir)
		try:
			us = Popen(split(cmd))
			us.communicate()
			if us.returncode == 0:
...
into the directory specified with -d.")
	...
	if args.ublastdb:
		...
		if args.blast == True:
			...
			ublast(args.i, args.o, args.d, args.p)
